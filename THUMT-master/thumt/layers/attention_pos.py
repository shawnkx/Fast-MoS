...
def multiplicative_attention(queries, keys, values, k_pos, v_pos, bias, keep_prob=None,
                             name=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
        ...
        if k_pos is not None:
            ...
            q_len = l_shape[2]
            k_len = l_shape[3]
            q_dim = tf.shape(queries)[-1]

            queries = tf.reshape(tf.transpose(queries,[2,0,1,3]),[q_len,batch_size*num_heads,q_dim])
            logits +=  tf.reshape(tf.transpose(tf.matmul(queries, k_pos, transpose_b=True),[1,0,2]),tf.shape(logits))
        if bias is not None:
            logits += bias
        ...
        if v_pos is not None:
            weights = tf.reshape(tf.transpose(weights,[2,0,1,3]),[q_len,batch_size*num_heads,k_len])
            outputs += tf.reshape(tf.transpose(tf.matmul(weights, v_pos),[1,0,2]), tf.shape(outputs))       
        return {"weights": weights, "outputs": outputs}


def multihead_attention(queries, memories, bias, num_heads, key_size,
                        value_size, output_size, keep_prob=None, output=True,
                        state=None, dtype=None, scope=None, k_size=0):
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        v_pos = None
        if k_size:
            pos_rep_kv = tf.get_variable("pos_kv_emb", [2*k_size, (key_size+value_size)//num_heads], dtype=dtype)
            #pos_rep_v = tf.get_variable("pos_v_emb", [2*k_size, value_size//num_heads], dtype=dtype)
            ...
            if state is not None:
                indices = tf.maximum(tf.minimum(tf.range(k_size,k_size+len_k)-(len_k-1), 2*k_size-1),0)
            else:
                indices = tf.maximum(tf.minimum(tf.tile(tf.range(k_size,k_size+len_k),[len_q]) - tf.reshape(tf.tile(tf.reshape(tf.range(len_q),[len_q,1]),[1,len_k]), [len_q*len_k]), 2*k_size-1),0)    
            #kv_pos = tf.reshape(tf.gather(pos_rep_kv,indices),[1,1,len_q,len_k,(key_size+value_size)//num_heads])
            kv_pos = tf.reshape(tf.gather(pos_rep_kv,indices),[len_q,len_k,(key_size+value_size)//num_heads])
            #v_pos = tf.reshape(tf.gather(pos_rep_v,indices),[1,1,len_q,len_k,value_size//num_heads])
            #kv_pos = tf.tile(kv_pos,[shape_q[0],num_heads,1,1,1])
            k_pos = kv_pos[:,:,:key_size//num_heads]
            v_pos = kv_pos[:,:,key_size//num_heads:]
            #v_pos = tf.tile(v_pos,[shape_q[0],num_heads,1,1,1])    
        ...
        # attention
        results = multiplicative_attention(q, k, v, k_pos, v_pos, bias, keep_prob)
