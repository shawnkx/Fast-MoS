...
def _prior_layer(inputs, output_size, keep_prob=None, dtype=None, scope=None):
    with tf.variable_scope(scope, default_name="prior_layer", values=[inputs], dtype=dtype):
        output = tf.layers.dense(inputs, output_size, use_bias=False,
            kernel_initializer=tf.initializers.uniform_unit_scaling)
        return output

def _latent_layer(inputs, output_size, keep_prob=None, dtype=None, scope=None):
    with tf.variable_scope(scope, default_name="latent_layer", values=[inputs], dtype=dtype):
        output = tf.layers.dense(inputs, output_size, activation=tf.nn.tanh,
            kernel_initializer=tf.initializers.uniform_unit_scaling)
        return output
...
def encoding_graph(features, mode, params):
    #features["source"] = tf.Print(features["source"], [tf.shape(features["source"])], "Value of x is: ")
    #features["source_length"] = tf.Print(features["source_length"], [tf.shape(features["source_length"])], "Value of x is: ")
    if mode != "train":
...
def decoding_graph(features, state, mode, params):
    ...
    if mode != "infer":
        ...
        #decoder_output = tf.Print(decoder_output, [tf.shape(decoder_output)], 'infer mode')
        
        if params.mos:
            latent_w = tf.get_variable("latent_w",
                            [hidden_size, params.n_experts * hidden_size],
                            initializer=initializer)
            latent_bias = tf.get_variable("latent_b",
                            [params.n_experts * hidden_size],
                            initializer=initializer)
            #latent = _latent_layer(decoder_output, params.n_experts * hidden_size)
            latent = tf.nn.tanh(tf.add(tf.matmul(decoder_output, latent_w), latent_bias))
            #latent = tf.layers.dense(decoder_output, params.n_experts * hidden_size,
                #activation=tf.nn.tanh, name='latent')
            #latent = tf.Print(latent, [tf.shape(latent)], "shape of latent is: ")
            latent = tf.reshape(latent, [-1, hidden_size])
            #latent = tf.Print(latent, [tf.shape(latent)], "shape of re-latent is: ")
            logits = tf.matmul(latent, weights, False, True)
            #prior = _prior_layer(decoder_output, params.n_experts)
            prior_w = tf.get_variable("prior_w",
                            [hidden_size, params.n_experts],
                            initializer=initializer)
            #prior = tf.layers.dense(decoder_output, params.n_experts,
                #use_bias=False, name='prior')
            prior = tf.matmul(decoder_output, prior_w)
            prior = tf.nn.softmax(prior)
            #prior = tf.Print(prior, [tf.shape(prior)], "shape of prior is: ")
            logits = tf.nn.softmax(tf.reshape(logits, [-1, tgt_vocab_size]))
            #logits = tf.reshape(logits, [-1, tgt_vocab_size])
            logits = tf.reshape(logits, [-1, params.n_experts, tgt_vocab_size])
            prior = tf.expand_dims(prior, 2)
            prior = tf.tile(prior, [1, 1, tgt_vocab_size])
            #prior = tf.Print(prior, [tf.shape(prior)], "Value of x is: ")
            prob = prior * logits
            prob = tf.reduce_sum(prob, 1)
            logits = tf.reshape(prob, [-1, tgt_vocab_size])
    ...
    #decoder_output = tf.Print(decoder_output, [tf.shape(decoder_output)], "Value of x is: ")
    if params.mos:
        latent_w = tf.get_variable("latent_w",
                        [hidden_size, params.n_experts * hidden_size],
                        initializer=initializer)
        ...
        prob = tf.reduce_sum(prob, 1)
        logits = tf.reshape(prob, [-1, tgt_vocab_size])
        #logits = tf.Print(logits, [logits], "logits value is")
    else:
        logits = tf.matmul(decoder_output, weights, False, True)
    ...
    if params.mos:
        ce = layers.nn.mos_smoothed_softmax_cross_entropy_with_logits(
            logits=logits,
            labels=labels,
